...
![mindmap 변환 결과](images/09-mindmap.jpg)

뿌리 노드는 `root((...))` 문법 그대로 원형으로 그려지고, 들여쓰기 한 단계가 부모-자식 엣지 하나에 대응합니다.
...
![requirementDiagram 변환 결과와 노드 편집 패널](images/10-requirement.jpg)

요구사항은 `<<Requirement>>`, 요소는 `<<Element>>`처럼 종류를 나타내는 줄이 라벨 첫 줄에 붙고, `contains`나 `satisfies` 같은 관계 이름은 엣지 라벨로 표시됩니다.

작성할 때 한 가지 주의할 점이 있습니다. mermaid의 요구사항 문법은 이름과 값에 따옴표가 없으면 ASCII 낱말만 허용하므로, 한글이나 공백이 들어가는 이름과 값은 `text: "사용자는 이메일로 로그인한다"`처럼 큰따옴표로 감싸야 합니다. 감싸지 않으면 문법 오류로 표시됩니다.
...
노드-엣지 모델로 옮길 수 없는 유형은 mermaid가 그린 SVG를 그대로 보여 주는 미리보기 전용 모드로 전환됩니다. 화면 위쪽에 `미리보기 전용` 표시가 붙고, `노드 추가`와 `캔버스 → 코드` 버튼은 비활성화됩니다. 이미지 내보내기는 그대로 사용할 수 있습니다.

![미리보기 전용 모드](images/07-preview-only.jpg)

### 표로 편집하는 유형

`pie`, `gantt`, `journey`, `timeline`은 미리보기 아래에 행 단위 표가 함께 나옵니다. 화면 위쪽 표시가 `표 편집`으로 바뀌고, 표의 칸을 고치면 코드가 그 자리에서 갱신되며 미리보기도 다시 그려집니다. `+ 행 추가`로 마지막 행 아래에 새 행을 넣고, 행 오른쪽의 `×`로 그 행을 지웁니다.

표는 코드의 해당 줄 하나만 다시 쓰므로 `title`, `dateFormat`, `excludes` 같은 지시어와 구획(`section`) 줄은 그대로 남습니다. 간트의 속성 칸은 `a1, 2024-01-01, 30d`처럼 원문을 그대로 보여 주는데, `after a1` 같은 상대 날짜 표현을 잃지 않기 위해서입니다.

미리보기 전용 모드에서 `PNG`를 누르면 화면에 보이는 미리보기 영역을 그대로 이미지로 만들고, `SVG`를 누르면 mermaid가 만든 SVG를 그대로 내려받습니다. PNG는 미리보기 영역 전체를 캡처하므로 다이어그램 아래쪽 빈 공간까지 함께 담깁니다.
